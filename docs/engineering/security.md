...
- Trust display is client-calculated and non-authoritative. Firestore denies `trustScores` and reserved profile trust mutation; another user's card uses reviews-only evidence and does not read or infer private identity, account-age, cancellation, or participant history.
...
- Trust has no durable server evidence projection, appeals/corrections workflow, abuse monitoring, or bias/fairness review yet and must not drive authorization or safety claims.
