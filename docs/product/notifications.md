...
The local bus remains synchronous and non-durable for notification types other than `booking.accepted`. N3A does not add a general durable event system, retries, receipt polling, queues, scheduled delivery, monitoring dashboards, localization, or background workers.

N3A implements one immediate, kill-switched Expo provider attempt for a valid `booking.accepted` transition. It validates the complete persisted notification-preference schema and suppresses partial, malformed, mismatched, or unknown-key records before token lookup. It deterministically examines at most 100 device registration records for the recipient, claims no more than 100 delivery effects, and sends at most 100 messages in one Expo request. Registrations beyond that bound are ignored in this phase. Response-body timeout/abort and network failures are temporary outcomes, while malformed JSON is a permanent outcome; N3A does not retry either outcome. This cap is a safety and resource boundary, not a production-scale broadcast design. Broader device-retention policy, pagination, cleanup, and multi-batch fan-out remain deferred. N3A is not production-enabled or production-ready. Email and SMS remain unimplemented.
...
The token then passes through `PushRegistrationService`, which validates ownership and token shape before calling the repository port. N1/N2 persist it through authenticated trusted callables into a server-only registration collection. The token is not displayed, logged, returned by the callables, or written directly to Firestore by the client. Denial does not trigger another prompt or open system Settings.

The existing unregister path calls the authenticated N1 endpoint when explicitly invoked. No sign-out UI currently invokes it, and automatic logout/startup reconciliation remains deferred.

For N3A, the trusted booking update itself is the event boundary, and creation of its deterministic canonical notification is the event-level dispatch claim. Only the invocation that creates the record may evaluate the kill switch, preferences, quiet hours, or token state and continue into optional push. A matching duplicate returns `event_replay` without reevaluation, so quiet-hours, preference, kill-switch, and no-token suppression are terminal even when state later changes. The creating invocation derives the sender recipient and traveler actor from the booking, selects active bound Expo registrations from the first 100 document-ID-ordered records, claims a deterministic per-device effect, and makes at most one immediate provider request of no more than 100 messages. It does not read a second page or queue overflow registrations. A crash after canonical creation but before device claiming can lose the optional push; this is an accepted N3A at-most-one-attempt tradeoff because the in-app notification remains canonical. No retry, queue, delayed delivery, or historical catch-up is added.

Trusted token registration is implemented in N1. N3A immediately deactivates a registration only when Expo identifies it as `DeviceNotRegistered`. Token rotation, automatic sign-out removal, startup reconciliation, scheduled cleanup, retention, and broader lifecycle management remain deferred. Token values are sensitive operational data and must never enter domain events, notification/delivery records, analytics, or logs.
...
The N3A provider path is disabled unless the server-only `KARRI_PUSH_DELIVERY_ENABLED` value is exactly `true`. Production enablement and deployment remain deferred until native credentials, device validation, App Check rollout, broader token lifecycle handling, diagnostics, and operational approval are complete.
