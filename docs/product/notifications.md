...
Trusted token registration is implemented in N1, N3B adds generation maintenance and cleanup safety, N4A adds manual current-installation removal, and N4B adds bounded cleanup before sign-out. N4B captures the expected user, serializes auth and push operations, continues sign-out after cleanup failure or a three-second timeout, invalidates abandoned operations, and prevents late work from persisting into a later session. An immediate Expo `DeviceNotRegistered` result deactivates a registration only when the transaction still sees the same active token and `registrationVersion` selected for that send; a newer generation remains active. Automatic native token-change detection, startup/foreground reconciliation, permission-revocation cleanup, preference-disable cleanup, receipt polling, scheduled cleanup, retention, and broader lifecycle management remain deferred. Token values are sensitive operational data and must never enter domain events, notification/delivery records, analytics, or logs.
...
The N3A/N3B provider path is disabled unless the server-only `KARRI_PUSH_DELIVERY_ENABLED` value is exactly `true`. Registration-generation binding and N4B cleanup do not authorize sending. `registerPushToken`, `unregisterPushToken`, and `onBookingAccepted` are implemented in source but not deployed in development. Production enablement remains **No-Go** until native credentials, real-device validation, App Check rollout, receipt handling, broader token lifecycle management, diagnostics, and operational approval are complete.
